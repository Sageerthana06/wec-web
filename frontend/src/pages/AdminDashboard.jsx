...
const AdminDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      setLoading(true);
      const [messagesRes, servicesRes, productsRes, galleryRes] =
        await Promise.all([
  ...
  const toggleMessageStatus = async (id, currentStatus) => {
    try {
      ...
      if (res.success) {
        setMessages((prev) =>
          prev.map((msg) =>
            msg._id === id ? { ...msg, status: nextStatus } : msg,
          ),
        );
  ...
  const handleCompanyUpdate = async (e) => {
    ...
    try {
      ...
      formData.append(
        "socialLinks",
        JSON.stringify(editCompanyForm.socialLinks),
      );
    ...
    try {
      ...
      if (galleryFormMode === "add") {
        ...
        if (res.success) {
          ...
          setGallery((prev) =>
            prev
              .map((g) => (g._id === currentGalleryId ? res.data : g))
  ...
  if (authLoading || loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 pt-32 pb-24 text-center">
        ...
        <p className="mt-4 text-slate-400 animate-pulse-subtle">
          Accessing secure system data...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-200 dark:border-slate-800 pb-8 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-450 dark:text-slate-400 mt-1">
            Manage your company details, modify services and products lists, and
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <GlassCard
          hover={false}
          className="flex items-center gap-4 p-5 border border-slate-200 dark:border-slate-850"
        >
                  ...
                  >
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="flex flex-wrap justify-between items-start gap-4">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2.5">
                            <h4 className="font-extrabold text-base sm:text-lg text-slate-850 dark:text-white">
                              {msg.name}
                            </h4>
                            {unread && (
                      ...
                      <div className="flex items-center justify-end gap-3 mt-2 border-t border-slate-100 dark:border-slate-800/50 pt-3">
                        <button
                          onClick={() =>
                            toggleMessageStatus(msg._id, msg.status)
                          }
        ...
        <form onSubmit={handleCompanyUpdate} className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 flex flex-col gap-6">
              <GlassCard
                hover={false}
                className="flex flex-col gap-5 p-6 border border-slate-200 dark:border-slate-850"
              >
                <h3 className="font-bold text-base border-b border-slate-100 dark:border-slate-800 pb-2">
                  Public Branding
                </h3>
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              <GlassCard
                hover={false}
                className="flex flex-col items-center gap-4 p-6 border border-slate-200 dark:border-slate-850"
              >
                <h3 className="font-bold text-base border-b border-slate-100 dark:border-slate-800 pb-2 w-full text-center">
                  Company Logo
                </h3>
                ...
                {companyLogoFile ? (
                  <div className="w-24 h-24 rounded-2xl overflow-hidden bg-slate-100 flex items-center justify-center border border-dashed border-primary-500">
                    <span className="text-[10px] text-primary-500 font-bold p-1 text-center truncate">
              ...
              <GlassCard
                hover={false}
                className="flex flex-col gap-4 p-6 border border-slate-200 dark:border-slate-850"
              >
                <h3 className="font-bold text-base border-b border-slate-100 dark:border-slate-800 pb-2">
      ...
      {activeTab === "services" && (
        <div className="flex flex-col gap-6">
          ...
          {showServiceForm && (
            <GlassCard
              hover={false}
              className="border border-primary-500/35 p-6 mb-6"
            >
              <h3 className="font-bold text-lg mb-4 capitalize">
                {serviceFormMode} Service
              </h3>
              <form
                    ...
                    <div className="flex gap-4 mt-1">
                      <label className="flex items-center gap-1.5 text-sm cursor-pointer">
                        <input
                          ...
                          onChange={() =>
                            setServiceForm({ ...serviceForm, status: "active" })
                          }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {services.map((s) => (
              <GlassCard
                key={s._id}
                hover={false}
                className="flex flex-col justify-between border border-slate-200 dark:border-slate-800"
              >
                <div className="flex flex-col gap-4">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span
                          className={`text-[10px] font-extrabold uppercase ${s.status === "active" ? "text-emerald-500" : "text-slate-400"}`}
                        >
                          {s.status}
                        </span>
                      </div>
                    ...
                    <div className="flex gap-4 mt-1">
                      <label className="flex items-center gap-1.5 text-sm cursor-pointer">
                        <input
                          ...
                          onChange={() =>
                            setProductForm({ ...productForm, status: "active" })
                          }
                          className="accent-primary-500"
                        />
                        Active
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((p) => (
              <GlassCard
                key={p._id}
                hover={false}
                className="flex flex-col justify-between p-0 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 h-full"
              >
                ...
                <div className="p-5 flex flex-col justify-between flex-grow">
                  ...
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100 dark:border-slate-800/80">
                    <span
                      className={`text-[10px] font-extrabold uppercase ${p.status === "active" ? "text-emerald-500" : "text-slate-450"}`}
                    >
                      {p.status}
                    </span>
                    <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col gap-6">
            ...
            >
              ...
              >
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-bold text-slate-400 uppercase">
                      Upload Image File {galleryFormMode === "add" && "*"}
                    </label>
                    <input
                      type="file"
                      accept="image/*"
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {gallery.map((item) => (
              <div
                key={item._id}
                className="relative rounded-xl overflow-hidden aspect-square group shadow border border-slate-200 dark:border-slate-800 bg-slate-900"
              >
                <img
                ...
                <div className="absolute bottom-0 inset-x-0 p-3 bg-slate-950/70 backdrop-blur-xs text-white text-[10px] flex flex-col">
                  ...
                  <span className="text-[9px] text-primary-400 mt-0.5">
                    {item.category}
                  </span>
                </div>
              </div>
      ...
      {activeTab === "security" && (
        <div className="max-w-xl">
          <GlassCard
            hover={false}
            className="p-6 border border-slate-200 dark:border-slate-850"
          >
            <h3 className="font-bold text-lg border-b border-slate-100 dark:border-slate-800 pb-2 mb-5">
